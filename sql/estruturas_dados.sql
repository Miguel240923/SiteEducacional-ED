...
CREATE TABLE `conteudos` (
  `id` int(11) NOT NULL,
  `estrutura` enum('tad','lista_simples','lista_dupla','fila_fifo','fila_prioridade','pilha') NOT NULL COMMENT 'Qual estrutura esse conteúdo pertence',
...
-- ============================================================
-- TP3: usuários, gamificação, quiz e loja
-- ============================================================
CREATE TABLE IF NOT EXISTS usuarios (
  ...
  avatar_chapeu INT NULL,
  avatar_face INT NULL,
  avatar_roupa INT NULL,
  recuperacao_token VARCHAR(100) NULL,
  recuperacao_expira DATETIME NULL,
...
CREATE TABLE IF NOT EXISTS quiz_perguntas (
  ...
  estrutura VARCHAR(40) NOT NULL DEFAULT 'geral',
  ativo TINYINT(1) NOT NULL DEFAULT 1
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS loja_itens (
  ...
  categoria ENUM('chapeu','rosto','roupa','habilidade') NOT NULL,
...
CREATE TABLE IF NOT EXISTS inventario (
  ...
  UNIQUE KEY unq_inventario (usuario_id,item_id),
  FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  FOREIGN KEY (item_id) REFERENCES loja_itens(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

INSERT INTO loja_itens (nome,categoria,icone,preco,descricao,habilidade) VALUES
('Chapéu de Algoritmo','chapeu','🎩',50,'Chapéu clássico do estudante de algoritmos','+5% de moedas nas recompensas'),
('Coroa do Mestre','chapeu','👑',120,'Para quem concluiu muitos desafios','+10% de moedas nas recompensas'),
('Boné do Código','chapeu','🧢',80,'Visual descontraído','Dica extra em 1 questão por quiz'),
('Rosto Curioso','rosto','😀',0,'Rosto padrão',''),
('Rosto Nerd','rosto','🤓',40,'Óculos para estudar','+1 moeda por quiz'),
('Rosto Robô','rosto','🤖',70,'Modo máquina de código','+5 XP por quiz'),
('Camiseta C#','roupa','👕',60,'Camiseta de programação','+5% de moedas nas recompensas'),
('Moletom Estruturado','roupa','🧥',100,'Moletom oficial','+10 XP por quiz'),
('Jaleco do Professor','roupa','🥼',150,'Visual de professor','+15% de moedas nas recompensas')
ON DUPLICATE KEY UPDATE nome=VALUES(nome);

INSERT INTO quiz_perguntas (pergunta,alternativa_a,alternativa_b,alternativa_c,alternativa_d,correta,explicacao,estrutura) VALUES
('O que significa FIFO?','First In, First Out','First In, Final Out','Fast In, Fast Out','File In, File Out','A','O primeiro elemento inserido é o primeiro removido.','fila_fifo'),
('Qual estrutura segue LIFO?','Fila','Pilha','Fila de prioridade','Lista simples','B','Na pilha, o último elemento inserido é o primeiro removido.','pilha'),
('Em uma lista encadeada, o nó normalmente possui...','Somente um índice','Valor e referência para outro nó','Somente um valor fixo','Um banco de dados','B','O encadeamento é feito pelas referências entre nós.','lista_simples'),
('Qual operação insere no topo de uma pilha?','Enqueue','Push','Dequeue','Peek','B','Push adiciona um elemento ao topo.','pilha'),
('Qual operação remove da frente de uma fila?','Push','Pop','Dequeue','Peek','C','Dequeue remove o elemento que está no início da fila.','fila_fifo'),
('Uma fila de prioridades atende principalmente pela...','ordem alfabética','prioridade associada ao elemento','posição da memória','quantidade de nós','B','A prioridade determina a ordem de atendimento; empates podem seguir FIFO.','fila_prioridade'),
('Qual é uma vantagem de inserir no início de lista simplesmente encadeada?','O(1)','O(n²)','O(log n) sempre','Não é possível','A','Basta ajustar a referência da cabeça.','lista_simples'),
('Peek em uma pilha normalmente...','remove o topo','consulta o topo sem remover','limpa a pilha','insere no fim','B','Peek consulta o elemento do topo.','pilha'),
('O que aponta para o primeiro nó de uma lista?','tail obrigatoriamente','head','queue','stack','B','Head é a referência para o primeiro nó.','lista_simples'),
('Por que usar uma fila encadeada?','Para impedir referências','Para representar uma sequência dinâmica de nós','Para eliminar o FIFO','Para obrigar tamanho fixo','B','Os nós podem ser alocados dinamicamente e ligados por referências.','fila_fifo'),
('Em uma pilha encadeada, o topo aponta para...','o nó mais recente','o último nó removido','um índice do array','sempre null','A','O topo representa o elemento disponível para remoção.','pilha'),
('O que caracteriza uma estrutura de dados?','Um conjunto organizado para armazenar/manipular dados','Apenas uma interface gráfica','Somente um arquivo','Uma linguagem de programação','A','Estruturas organizam dados e operações para manipulá-los.','geral');

INSERT INTO quiz_perguntas (pergunta,alternativa_a,alternativa_b,alternativa_c,alternativa_d,correta,explicacao,estrutura) VALUES
('Considere: `pilha.Push(10); pilha.Push(20); pilha.Pop();`. Qual valor sai?','10','20','30','Nenhum','B','Pop remove o topo; 20 foi o último inserido.','pilha'),
('Considere uma fila com [10,20,30]. Após uma operação Dequeue(), qual sequência resta?','[30,20]','[10,30]','[20,30]','[]','C','Dequeue remove o primeiro elemento.','fila_fifo'),
('No código de uma lista encadeada, `novo.Proximo = head; head = novo;` faz o quê?','Remove o primeiro nó','Insere um nó no início','Ordena a lista','Percorre até o fim','B','O novo nó passa a apontar para o antigo head e vira a nova cabeça.','lista_simples'),
('Em uma fila de prioridades, se A tem prioridade 2 e B prioridade 5, qual é atendido primeiro quando maior número significa maior prioridade?','A','B','Ambos ao mesmo tempo','Depende do tamanho','B','B possui prioridade maior.','fila_prioridade');
