...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('#navMenu a').forEach(link => {
        if (new URL(link.href).pathname === location.pathname) { link.classList.add('ativo'); link.setAttribute('aria-current','page'); }
    });
    document.addEventListener('keydown', e => { if(e.key === 'Escape' && nav && toggle) { nav.classList.remove('aberto'); toggle.setAttribute('aria-expanded','false'); } });
